...
def init_audio_system():
    ...
    st.markdown("""
        ...
        if (window.simpleAudioSystem) {
            const success = window.simpleAudioSystem.init();
            if (success) {
                window.parent.postMessage({type: 'AUDIO_INIT_SUCCESS'}, '*');
            }
        }
        </script>
    """, unsafe_allow_html=True)

def load_audio_data(key, base64_data):
    st.markdown(f"""
        <script>
        if (window.simpleAudioSystem) {{
            window.simpleAudioSystem.loadSound('{key}', '{base64_data}');
        }}
        </script>
    """, unsafe_allow_html=True)
...
def play_sound(sound_key):
    st.markdown(f"""
        <script>
        if (window.simpleAudioSystem) {{
            window.simpleAudioSystem.play('{sound_key}');
        }}
        </script>
    """, unsafe_allow_html=True)
...
def main():
    ...
    # 初始化会话状态
    if 'audio_init_attempted' not in st.session_state:
        st.session_state.audio_init_attempted = False
        st.session_state.audio_initialized = False

    # 首次加载时注入音频系统
    if not st.session_state.audio_init_attempted:
        init_audio_system()
        st.session_state.audio_init_attempted = True
        ...
        load_audio_data('phase1', audio_data1)
        load_audio_data('phase2', audio_data2)
    ...
    with init_col2:
        if st.button("点击初始化音频", use_container_width=True):
            st.session_state.audio_initialized = True
            st.markdown("""
                <script>
                if (window.simpleAudioSystem) {
                    window.simpleAudioSystem.init();
                }
                </script>
            """, unsafe_allow_html=True)
            
            # 测试音效
            time.sleep(0.5)  # 短暂延迟确保初始化完成
            play_sound('phase1')
            st.rerun()
